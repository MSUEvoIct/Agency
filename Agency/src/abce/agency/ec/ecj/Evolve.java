/*
 * This Evolve class was copied from ECJ.  It was modified to include
 * the ability to change the parameter database using command line arguments
 * after loading it.
 */
 ...
 * 
 * <p>
 * <tt>java ec.Evolve -file </tt><i>parameter_file [</i><tt>-p </tt>
 * <i>parameter=value]*</i>
 * 
 * <p>
 * This starts a new evolutionary run, using the parameter file
 * <i>parameter_file</i>. The user can provide optional overriding parameters on
 * the command-line with the <tt>-p</tt> option.
 * 
 * <p>
 ...
 * facility. If you'd like to run the evolutionary system four times, each with
 * a different random seed, you might do:
 * 
 * <p>
 * <tt>java ec.Evolve -file </tt><i>parameter_file</i> <tt>-p jobs=4</tt>
 * 
 * <p>
 * Here, Evolve will run the first time with the random seed equal to whatever's
 * specified in your file, then job#2 will be run with the seed + 1, job#3 with
 * the seed + 2, and job#4 with the seed + 3. If you have multiple seeds, ECJ
 * will try to make sure they're all different even across jobs by adding the
 * job number * numberOfSeeds to each of them. This means that if you're doing
 * multiple jobs with multiple seeds, you should probably set seed.0 to x,
 * seed.1 to x+1, seed.2 to x+2, etc. for best results. It also works if seed.0
 * is x, seed.1 is y (a number much bigger than x), seed.2 is z (a number much
 * bigger than y) etc.
 * 
 * If you set seed.0=time etc. for multiple jobs, the values of each seed will
 * be set to the current time that the job starts plus the job number *
 * numberOfSeeds. As current time always goes up, this shouldn't be an issue.
 * However it's theoretically possible that if you checkpoint and restart on
 * another system with a clock set back in time, you could get the same seed in
 * a later job.
 * 
 * <p>
 * <b>main() has been designed to be modified.</b> The comments for the
 * Evolve.java file contain a lot discussion of how ECJ's main() bootstraps the
 * EvolutionState object and runs it, plus a much simpler example of main() and
 * explanations for how main() works.
 * 
 * 
 * <p>
 * <b>Parameters</b><br>
 ...
 * <td valign=top>(The number of jobs to iterate. The current job number
 * (0...jobs-1) will be added to each seed UNLESS the seed is loaded from the
 * system time. The job number also gets added as a prefix (if the number of
 * jobs is more than 1)).</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><tt>nostore</tt><br>
 * <font size=-1> bool = <tt>true</tt> or <tt>false</tt> (default)</font></td>
 * <td valign=top>(should the ec.util.Output facility <i>not</i> store
 * announcements in memory?)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><tt>flush</tt><br>
 * <font size=-1> bool = <tt>true</tt> or <tt>false</tt> (default)</font></td>
 * <td valign=top>(should I flush all output as soon as it's printed (useful for
 * debugging when an exception occurs))</td>
 * </tr>
 * 
 * <tr>
 ...
 * <td valign=top>(the seed for random number generator #<i>n</i>. <i>n</i>
 * should range from 0 to Max(evalthreads,breedthreads)-1. If value is
 * <tt>time</tt>, then the seed is based on the system clock plus <i>n</i>.)</td>
 * </tr>
 * 
 * <tr>
 ...
 * <td valign=top>(at the end of a run, do we print out a list of all the
 * parameters requested during the run?)</td>
 * </tr>
 * 
 * <tr>
 ...
 * <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</td>
 * <td valign=top>(at the end of a run, do we print out a list of all the
 * parameters actually <i>used</i> during the run?)</td>
 * </tr>
 * 
 * <tr>
 ...
 * <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</td>
 * <td valign=top>(at the end of a run, do we print out a list of all the
 * parameters NOT requested during the run?)</td>
 * </tr>
 * 
 * <tr>
 ...
 * <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</td>
 * <td valign=top>(at the end of a run, do we print out a list of all the
 * parameters NOT actually used during the run?)</td>
 * </tr>
 * 
 * <tr>
 ...
 * <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</td>
 * <td valign=top>(at the end of a run, do we print out a list of all the
 * parameters stored in the parameter database?)</td>
 * </tr>
 * 
 * </table>
 * 
...
public class Evolve
{
	...
	public static ParameterDatabase loadParameterDatabase(String[] args)
	{
		...
		updateParameterDatabase(args, parameters);
		return parameters;
	}
	...
	/**
	 * This method updates the parameter database for ECJ by taking command line
	 * arguments.
	 * For example
	 * -eset seed.0 100
	 * Will set the parameter seed.0 to 100
	 * 
	 * @param args
	 * @param p
	 */
	public static void updateParameterDatabase(String[] args, ParameterDatabase p) {
		...
		Output.initialMessage(sb.toString());
		...
		while (ndx < nargs) {
			...
			if (tok.equals("-eset")) {
				if (ndx + 2 < nargs) {
					path = args[ndx + 1];
					setting = args[ndx + 2];
					p.set(new Parameter(path), setting);
					ndx += 3;
				} else {
					Output.initialError("Invalid parameter database setting.  Should be in form -eset Path Value");
				}
			} else {
				ndx++;
			}
	 ...
	 * a basic
	 * linear congruential generator; thereafter it uses the MersenneTwister
	 * algorithm to
	 * build new seeds. Those first 624 numbers are generally just fine, but to
	 * be extra
	 * safe, you can prime the generator by calling nextInt() on it some (N>1) *
	 * 624 times.
	 ...
	 * seedParameter. If the parameter
	 * is V_SEED_TIME, the seed is set to the currentTime value. Then the seed
	 * is incremented by the offset.
	 * This method is broken out of initialize(...) primarily to share code with
	 ...
	 * adjustment (added to each random seed).
	 * The adjustment offers a convenient way to change the seeds of the random
	 * number generators each time you
	 * do a new evolutionary run. You are of course welcome to replace the
	 * random number generators after initialize(...)
	...
	public static EvolutionState initialize(ParameterDatabase parameters, int randomSeedOffset, Output output)
	{
		...
		boolean auto = (V_THREADS_AUTO.equalsIgnoreCase(parameters.getString(new Parameter(P_BREEDTHREADS), null)) ||
				V_THREADS_AUTO.equalsIgnoreCase(parameters.getString(new Parameter(P_EVALTHREADS), null))); // at
																											// least
																											// one
																											// thread
																											// is
		...
		for (x = 0; x < random.length; x++)
		{
			...
			random[x] = Evolve.primeGenerator(new MersenneTwisterFast(seeds[x])); // we
																					// prime
																					// the
																					// generator
																					// to
	 ...
	 * been
	 * provided by initialize(...). The jobPrefix is added to the front of
	 * output and
	 ...
	 * MAIN
	 * 
	 * Evolve has... evolved from previous Evolves. The goal behind these
	 * changes is:
	 ...
	 * 
	 * Before we get into the specifics of this file, let's first look at the
	 * main
	 ...
	 * 
	 * That's all there's to it. Various EvolutionState classes need to
	 * implement
	 * the startFromCheckpoint, startFresh, evolve, and finish methods. This
	 * basic
	 * evolution loop is encapsulated in a convenience method called
	 * EvolutionState.run(...).
	 * 
	 * Evolve.java is little more than code to fire up the right EvolutionState
	 * class,
	 * call run(...), and then shut down. The complexity mostly comes from
	 * bringing
	 * up the class (loading it from checkpoint or from scratch) and in shutting
	 * down.
	 ...
	 * a checkpoint file to load from. Else it returns the unfrozen
	 * EvolutionState.
	 * 
	 * - To instead set up from scratch, you have to do a bunch of stuff to set
	 * up the state.
	 * First, you need to load a parameter database. Evolve.java has a
	 * convenience function
	 * for that called loadParameterDatabase(...). Second, you must do a series
	 * of items: (1) generate an Output object (2) identify the number of
	 * threads
	 * (3) create the MersenneTwisterFast random number generators (4)
	 * instantiate
	 * the EvolutionState subclass instance (5) plug these items, plus the
	 * random
	 ...
	 * requested a printout at the end [rarely] (3) flush System.err and
	 * System.out
	 * for good measure, and (4) close Output -- which closes its streams except
	 * for System.err and System.out. There is a convenience function for this
	 * as
	 * well. It's called cleanup(...).
	 * 
	 * - Last, you shut down with System.exit(0) -- very important because it
	 * quits
	 ...
	 * Piece of cake!
	 * 
	 * The more extravagant main(...) you see below just has a few extra gizmos
	 * for
	 ...
	 * 
	 * The reason these are slots in EvolutionState is so you can store this
	 * information
	 * across checkpoints and continue where you had started job-number-wise
	 * when the
	 * user starts up from a checkpoint again.
	 * 
	 * You'll probably want the EvolutionState to output its stat files etc.
	 * using unique
	 * prefixes to differentiate between jobs (0.stat, 1.stat, or whatever you
	 * like -- it
	 * doesn't have to be numbers), and you'll also probably want checkpoint
	 * files to be
	 ...
	 * 
	 * The extravagant main below is basically doing this. We're using state.job
	 * to stash
	 * away a single iterated job number, stored as an Integer in state.job[0],
	 * and then
	 * iterating that way, making sure we stash the job number and runtime
	 * arguments each time
	 * so we can recover them when loading from checkpoint. We use the "jobs"
	 * parameter
	 * to determine how many jobs to run. If this number is 1, we don't even
	 * bother to set
	 * the file prefixes, so ECJ generates files just like it used to.
	 * 
	 * It's important to note that this main was created with the assumption
	 * that you might
	 * modify it for your own purposes. Do you want a nested loop, perhaps to do
	 * all combinations
	 * of two parameters or something? Rewrite it to use two array slots in the
	 * job array.
	 * Want to store more information on a per-job basis? Feel free to use the
	 * job array any
	...
	public static void main(String[] args)
	{
		...
		// if we're loading from checkpoint, let's finish out the most recent
		// job
		state = possiblyRestoreFromCheckpoint(args);
		int currentJob = 0; // the next job number (0 by default)

		// this simple job iterator just uses the 'jobs' parameter, iterating
		// from 0 to 'jobs' - 1
		// inclusive. The current job number is stored in state.jobs[0], so
		// we'll begin there if
		// we had loaded from checkpoint.

		if (state != null) // loaded from checkpoint
		{
			// extract the next job number from state.job[0] (where in this
			...
			try
			{
				...
				args = state.runtimeArguments; // restore runtime arguments from
												// checkpoint
		...
		// A this point we've finished out any previously-checkpointed job. If
		// there was
		// one such job, we've updated the current job number (currentJob) to
		// the next number.
		...
		// Now we know how many jobs remain. Let's loop for that many jobs. Each
		// time we'll
		// load the parameter database scratch (except the first time where we
		// reuse the one we
		// just loaded a second ago). The reason we reload from scratch each
		// time is that the
		// experimenter is free to scribble all over the parameter database and
		// it'd be nice to
		// have everything fresh and clean. It doesn't take long to load the
		// database anyway,
		// it's usually small.
		for (int job = currentJob; job < numJobs; job++)
		{
			// We used to have a try/catch here to catch errors thrown by this
			// job and continue to the next.
			// But the most common error is an OutOfMemoryException, and
			// printing its stack trace would
			// just create another OutOfMemoryException! Which dies anyway and
			// has a worthless stack
			// trace as a result.

			// try
			{
				// load the parameter database (reusing the very first if it
				// exists)
				if (parameters == null)
					parameters = loadParameterDatabase(args);
									...
									// backwards-compatible),
				{
					...
					state.output.setFilePrefix(jobFilePrefix); // add a prefix
																...
																// checkpoint/output
																// files
					state.checkpointPrefix = jobFilePrefix + state.checkpointPrefix; // also
																						// set
																						// up
																						// checkpoint
																						// prefix
